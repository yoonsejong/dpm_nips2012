function [model_init] = get_init_value_m(type, mm_trans, M, vari, J, D_PARAM, MissIDX)
% GET_INIT_VALUE_M  Get initial value model for PPCA w/ missing values
%
% INPUT
...
elseif strcmp(type, 'sfm_d')
    ...
    for idj = 1:J
        ...
        model_init.W(:,:,idj) = ...
            [ mm_trans( past_frames + 1 : past_frames + 2, : ) + randn(2,N)/vari; ones(1,N)]';
        % cancel out if missing
%         NcI = (MissIDX( past_frames + 1, :) == 0);
%         model_init.W(NcI,:,idj) = repmat([ ones(2,1)/vari; 1 ]', [sum(NcI), 1]);
    end
    % MU should be initialized as zero
    model_init.MU = zeros(N, 1);
    % VAR is just a small number
...
elseif strcmp(type, 'sfm_dr')
    ...
    model_init.MU = zeros(N, 1);
    % VAR is just a small number
    model_init.VAR = rand(1) + D_PARAM;
elseif strcmp(type, 'd_dup')
    %----------------------------------------------------------------------
    % Simply duplicate centralized model over all nodes
    %----------------------------------------------------------------------
    if nargin < 5
        error('Did not specified number of nodes!');
    elseif nargin < 6
