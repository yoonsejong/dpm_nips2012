...
%  on     2012.02.05 (last modified on 2012/05/31)
%--------------------------------------------------------------------------
...
for idm = 1:length(MissIDXs)
    ...
    % Get initial values
    m_init_c = get_init_value_m('sfm_c', mm_trans, M, 10, 1, 0.5, MissIDX);

    % Note: we cannot use SVD-base PPCA for data with missing values.
    cm_c = cppca_em_m(mm_trans_t, MissIDX_t, M, THRESHc, m_init_c);
    ...
    parfor idr2 = 1:repeats % independent runs (initial value)
        m_init = get_init_value_m('sfm_dr', mm_trans_t', M, 10, J, 0.5, MissIDX_t');

        % D-PPCA
        V = get_sample_assign(J, D, 1); % node assignment of samples(=frames)
        E = get_adj_graph(J); E = E{2}; % graph topology (1 = compete, 2 = ring)
        cm_dppca = dppca_t_m(mm_trans_t, MissIDX_t, V, E, M, ETA, THRESHd, m_init, objfreq_d);
