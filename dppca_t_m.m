function [model] = dppca_t_m(X, MissIDX, V, E, M, ETA, THRESH, model_init, iter_obj)
% DPPCA_T_M    Distributed Probablistic PCA (D-PPCA) (Transposed)
% 
% Description
...
if nargin<7
    ...
    Wi = zeros(D, M, J);
    for idj = 1 : J
        Wi(:,:,idj) = model_init.W(:,:,idj);
    end
    MUi = repmat(model_init.MU ,[1, J]);
    if length(model_init.VAR) > 1 
        if length(model_init.VAR) == J
            PRECi = 1./model_init.VAR;
        else
            error('Incorrect initial VAR vector length!');
        end
    else
        PRECi = repmat(1./model_init.VAR, [J 1]);
    end
end
LAMBDAi = zeros(D,M,J);
GAMMAi = zeros(D,J);
...
while counter <= COUNTER_MAX
    %----------------------------------------------------------------------
    % temporarily save model...(for iteration visualization)
%     if nargin == 9
%         temp_save(sprintf('%s/iter_%05d.mat', SAVE_PATH, counter), ...
%             counter, J, Xi, Wi, MUi, PRECi, EZ, EZZt);
%     end
    ...
    for idx = 1 : J
        % Update (Eq. 18, 19, 37, 36, 34)
        [Wnew, MUnew, PRECnew, EZn, EZnZnt, Fnew] = ...
            dppca_t_m_local(Xi{idx}, MissIDXi{idx}, M, idx, Bj{idx}, ETA, ...
    ...
    if abs(relErr) < THRESH
        converged = 1;
        break;
    elseif 0*relErr > 0 %% <----- REMOVE 0* ONCE ALL BUGS ARE OUT
        error('Something is wrong -- objective function increased!');
    end
...
%--------------------------------------------------------------------------
% Temporarily save model - for visualization purpose
function temp_save(path, counter, J, Xi, Wi, MUi, PRECi, EZ, EZZt)
...
% Create structure
model = structure(W, MU, VAR, EZ, EZZt);
